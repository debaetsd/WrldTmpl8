...
void World::Commit()
{
	...
	for (uint j = 0; j < BRICKCOUNT / 32; j++) if (IsDirty32( j ) /* if not dirty: skip 32 bits at once */) 
	{
		for (uint k = 0; k < 32; k++)
		{
			...
			*brickIndices++ = i; // store index of modified brick at start of commit buffer
			StreamCopy( (__m256i*)changedBricks, (__m256i*)(brick + i * BRICKSIZE), BRICKMEMSIZE);
			changedBricks += BRICKMEMSIZE, tasks++;
		}
		ClearMarks32( j );
	...
	if (tasks > 0 || firstFrame)
	{
		...
		const uint copySize = firstFrame ? commitSize: (gridSize + MAXCOMMITS * 4 + tasks * BRICKMEMSIZE);
